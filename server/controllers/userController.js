...
const login = async (req, res) => {
  try {
    ...
    if (!isUserExists) {
      return res.status(404).send({ success: false, msg: "User not exists" });
    }
...
const addToFevorates = async (req, res) => {
  try {
    ...
    let addItem = await User.updateOne(
      { _id: user._id },
      { $push: { fevorates: product } }
    );
    if (addItem.modifiedCount === 1) {
      res.status(200).send({ success: true, msg: "Item added successfully" });
    } else {
      res
        .status(400)
        .send({ success: false, msg: "Failed to add item to favorites" });
    }
  } catch (err) {
    res.status(500).send(err.message);
  }
...
const removeFromFevorates = async (req, res) => {
  try {
    ...
    if (addItem.modifiedCount === 1) {
      ...
      res
        .status(400)
        .send({ success: false, msg: "Failed to remove item from favorites" });
    }
  } catch (err) {
    res.status(500).send(err.message);
  }
...
const addToCart = async (req, res) => {
  try {
    ...
    if (addItem.modifiedCount === 1) {
      ...
      res
        .status(400)
        .send({ success: false, msg: "Failed to add item to cart" });
    }
  } catch (err) {
    res.status(500).send(err.message);
  }
...
const removeFromCart = async (req, res) => {
  try {
    ...
    if (addItem.modifiedCount === 1) {
      ...
      res
        .status(400)
        .send({ success: false, msg: "Failed to remove item from cart" });
    }
  } catch (err) {
    res.status(500).send(err.message);
  }
...
const increaseQty = async (req, res) => {
  try {
    ...
    let increaseQuantity = await User.updateOne(
      { _id: req.userData._id, "cart._id": productId },
      { $inc: { "cart.$.quantity": +1 } }
    );
    if (increaseQuantity.modifiedCount === 1) {
      res.status(200).send({ success: true, msg: "Item quantity decreased" });
    } else {
...
const decreaseQty = async (req, res) => {
  try {
    const { productId } = req.body;
    let decreaseQuantity = await User.updateOne(
      { _id: req.userData._id, "cart._id": productId },
      { $inc: { "cart.$.quantity": -1 } }
    );
    if (decreaseQuantity.modifiedCount === 1) {
      res.status(200).send({ success: true, msg: "Item quantity decreased" });
    } else {
      res.status(200).send({ success: false, msg: "Something went wrong" });
    }
...
const resetPasswordClient = async (req, res) => {
  try {
    ...
    if (checkToken) {
      let verifyToken = await schema.findOne(
        { _id: id, "resetPasswordLinks.token": token },
        { "resetPasswordLinks.$": 1 }
      );
      if (verifyToken.resetPasswordLinks[0].expire !== true) {
        res.render("resetPassword", { id: id, token: token });
        let updateToken = await schema.updateOne(
          { _id: id, "resetPasswordLinks.token": token },
          { $set: { "resetPasswordLinks.$.expire": true } }
        );
      } else {
        res.render("linkUsed");
      }
...
const resetPassword = async (req, res) => {
  try {
    ...
    let hashPassword = await bcryptjs.hash(password, 12);
    let changeUserPassword = await schema.updateOne(
      { _id: id },
